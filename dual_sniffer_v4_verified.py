#!/usr/bin/env python3
"""
Использует pygeomag для точного расчета склонения (~7.4° для Калининграда).
Логика: Коса всегда перпендикулярна направлению носа устройства (оси X).
Если нос направлен по ходу движения -> Коса перпендикулярна треку GPS.
"""
...
def parse_nmea_extended(line: str, sys_time: float, declination_override: Optional[float] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if mag_data and 'x' in mag_data:
            # Магнитный азимут: угол вектора поля относительно оси X устройства
