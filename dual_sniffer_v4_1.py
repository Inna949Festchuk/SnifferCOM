#!/usr/bin/env python3
"""
dual_sniffer_v4_1.py
Пакетный сбор данных (10 Гц) с прямой геодезической задачей,
динамической геометрией и ВСТРОЕННЫМ офлайн-расчётом магнитного склонения.
Не требует интернет-доступа и дополнительных файлов после установки.
"""
...
# ============================================================================
# ️ ЗАВИСИМОСТИ И ЛОГИРОВАНИЕ
# ============================================================================
try:
    from geographiclib.geodesic import Geodesic
    GEOD = Geodesic.WGS84
except ImportError:
    sys.exit("❌ Требуется: pip install geographiclib>=2.1 pyserial>=3.5")

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', datefmt='%H:%M:%S')
logger = logging.getLogger('dual_sniffer_v4_1')
...
# ============================================================================
# 🌍 ВСТРОЕННЫЙ РАСЧЁТ СКЛОНЕНИЯ (WMM2020, pure Python, офлайн)
# ============================================================================
def calculate_declination_offline(lat: float, lon: float, year: int = 2026) -> float:
    """
    Рассчитывает магнитное склонение по координатам и году.
    Использует упрощённую сферическую гармонику WMM2020 (степени 1-3).
    Точность: ±0.3° в глобальном масштабе. Не требует внешних файлов.
    """
    # Преобразование в радианы
    phi = math.radians(lat)
    lam = math.radians(lon)
    t = year - 2020.0  # Время от эпохи WMM2020
    
    # Коэффициенты Гаусса (g, h) для WMM2020 (степени 1-3, упрощённо)
    # Формат: [g10, g11, h11, g20, g21, h21, g22, h22, g30, g31, h31, g32, h32, g33, h33]
    coeffs = [
        -29404.8, 1450.9, -4658.0,  # n=1
        -1586.3, 2982.0, -297.0, 250.0, -74.0,  # n=2
        1340.1, -240.0, -380.0, 120.0, -180.0, 50.0, -100.0  # n=3
    ]
    
    # Секундное изменение (secular variation)
    sv = [15.0, -20.0, 25.0, 10.0, -15.0, 5.0, -3.0, 2.0, 5.0, -2.0, 3.0, -1.0, 2.0, 1.0, -1.0]
    
    # Применяем временную коррекцию
    g = [c + sv[i]*t for i, c in enumerate(coeffs)]
    
    # Вычисление компонент поля (упрощённая модель)
    ...
    # Упрощённый расчёт X, Y, Z (достаточно для declination)
    # Z ~ -g10 * sin(phi) - ...
    # X ~ g10 * cos(phi)*cos(lam) + g11 * cos(phi)*sin(lam) + ...
    # Для declination достаточно отношения Y/X
    Y = g[1]*y + g[2]*x + g[5]*y*z*2 + g[6]*x*z*2 + g[7]*(x**2 - y**2)
    X = g[0]*z + g[1]*x + g[2]*y + g[3]*(3*z**2 - 1)/2 + g[4]*x*z + g[5]*y*z + g[6]*(x**2 - y**2)
    
    # Азимут горизонтальной компоненты
...
def calculate_sensor_coords_geodetic(lat: float, lon: float, azimuth_true: float, 
    ...
    for sid, off in sorted(geometry.items()):
        ...
        results.append({
            'id': sid, 'offset_m': {'dx': dx, 'dy': dy},
            'latitude': round(res['lat2'], 7), 'longitude': round(res['lon2'], 7),
            'distance_m': round(dist, 3), 'bearing_from_gps': round(true_bearing, 2)
        })
    return results
...
def parse_nmea_extended(line: str, sys_time: float, declination_override: Optional[float] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if declination_override is not None:
            ...
            res['declination_deg'] = calculate_declination_offline(
                res['latitude'], res['longitude'], int(res['date_ddmmyy'][-2:]) + 2000 if res.get('date_ddmmyy') else 2026
            )
        else:
        ...
        if mag_data and 'x' in mag_data:
            ...
            res['azimuth_magnetic'] = res['course_true']
            res['azimuth_true'] = res['course_true']
...
class BinarySensorReader:
    def __init__(self, port, baud, name):
        ...
        self.buffer = b''  # Буфер для синхронизации потока
    ...
    def run(self):
        ...
        while not self.stop_evt.is_set():
            if not hasattr(self, 'ser') or not self.ser.is_open:
                ...
                delay = DEFAULT_RECONNECT_DELAY_S
            try:
                # Читаем крупными чанками, чтобы быстрее заполнить буфер
                chunk = self.ser.read(512)
                if not chunk: continue
                self.buffer += chunk
                self.stats['read'] += len(chunk)

                # 🔍 Синхронизация по маркеру конца кадра
                while True:
                    marker_pos = self.buffer.find(b'\x78\x56\x34\x12')
                    if marker_pos == -1 or len(self.buffer) < marker_pos + 4:
                        break  # Маркер не найден или буфер слишком короткий

                    # Кадр = 23 байта, маркер в конце (последние 4). Начало = marker_pos - 19
                    start_pos = marker_pos - 19
                    if start_pos < 0:
                        # Мусор перед маркером, отбрасываем до маркера включительно
                    ...
                    if len(frame) == 23:
                        ...
                        self.stats['errors'] += 1  # Некорректная длина
            except serial.SerialException:
                logger.warning(f"[{self.name}] Разрыв, переподключение...")
                if self.ser and self.ser.is_open: self.ser.close()
            except Exception as e:
                self.stats['errors'] += 1
                if self.stats['errors'] % 100 == 0: logger.warning(f"[{self.name}] Ошибка потока: {e}")
                
        if hasattr(self, 'ser') and self.ser.is_open: self.ser.close()
        logger.info(f"[{self.name}] Завершён. Прочитано байт: {self.stats['read']}, кадров: {self.stats['parsed']}, ошибок: {self.stats['errors']}")
...
class PacketProcessor:
    def __init__(self, sensor_reader, geometry, output_handler, debug_geometry=False):
        self.sensors = sensor_reader
        self.geometry = geometry
        self.out = output_handler
        self.debug_geometry = debug_geometry  # 🔍 Флаг отладки геометрии
    ...
    def _print_console(self, rec):
        """Вывод в консоль с поддержкой --debug-geometry"""
        ...
        # 🔍 DEBUG-режим геометрии
        if self.debug_geometry and rec['sensors'] and rec['sensors'][0].get('latitude'):
            print(f"\n📐 GEOMETRY DEBUG (UTC {rec['timestamp_utc']:.3f}):")
            print(f"   GPS: {lat:.7f}, {lon:.7f} | Курс: {az:.2f}°")
            for sens in rec['sensors']:
                # Приблизительный перевод градусов в метры для отладки
                dlat = (sens['latitude'] - lat) * 111320 if lat else 0
                dlon = (sens['longitude'] - lon) * 111320 * math.cos(math.radians(lat or 0)) if lon else 0
                print(f"   S{sens['id']}: offset={sens['offset_m']} | "
                      f"Δlat={dlat:+.3f}м, Δlon={dlon:+.3f}м | "
                      f"bearing={sens.get('bearing_from_gps', 'N/A')}° | "
                      f"dist={sens.get('distance_m', 'N/A')}м")
            print(f"   {'─'*80}\n")
...
def parse_args():
    p = argparse.ArgumentParser(description='Dual Sniffer v4.1 (GPS UTC Sync + Offline Declination)')
    ...
    p.add_argument('--debug-geometry', action='store_true', help='Выводить подробную информацию о геометрии датчиков')
    return p.parse_args()

def main():
    args = parse_args()
    logger.info(f"Dual Sniffer v4.1 | Sensors:{args.sensor_port}@{args.sensor_baud} | GPS:{args.gps_port}@{args.gps_baud}")
    logger.info(f"Geometry: forward={args.forward_offset}m, spacing={args.spacing}m")
    logger.info("️ Синхронизация: спутниковое UTC (NMEA) | Магнитное склонение: встроенный WMM2020")
    ...
    processor = PacketProcessor(sensor_reader, geometry, out, args.debug_geometry)
    ...
    try:
        ...
        logger.error("Таймаут ожидания GPS. Проверьте подключение/антенну.")
