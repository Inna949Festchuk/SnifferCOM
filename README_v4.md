...
Каждый датчик (1–4) непрерывно выдаёт кадры фиксированной длины. Структура кадра:

| Смещение | Размер | Поле                | Тип       | Примечание                              |
|----------|--------|---------------------|-----------|----------------------------------------|
| 0        | 1      | Синхро              | uint8     | Всегда `0x00`                          |
| 1        | 1      | Индекс датчика  | uint8     | `0x00`                 |
| 2        | 1      | **Адрес отправителя**   | uint8     | `0x01` or `0x02` or `0x03` or `0x04`                          |
...
1. Байты накапливаются в кольцевой буфер.
...
1. Из `BinarySensorReader.latest` читаются последние доступные данные всех четырёх сенсоров.
...
# 🚀 Развернтывание и работа с системой.
---
...
2. Остаточные эффекты калибровки (см. ниже):
- Hard-iron (постоянное смещение): ±2–3°
- Soft-iron (искажение поля корпусом): ±1–2°
- Локальные аномалии (арматура, провода): ±3–5°

**Как уменьшить отклонение (если хочется)?Вариант: Медианный фильтр азимута**
...
2. **Порядок в протоколе** (фрагмент бинарного кадр 23 байта):
| Смещение | Размер | Поле                | Тип       | Примечание                              |
|----------|--------|---------------------|-----------|----------------------------------------|
| 0        | 1      | Синхро              | uint8     | Всегда `0x00`                          |
| 1        | 1      | Индекс датчика  | uint8     | `0x00`                 |
...
📐 GEOMETRY DEBUG (UTC 1777634537.138):
   ...
   ────────────────────────────────────────────────────────────────────────
✅ #00305 UTC=1778702728.164 | 📍 54.74161,20.50092 | 🧭 264.4° | Decl: 7.4° | датчиков: 4 | амплитуда (нТ): 123935317.9 | 188956662.1 | 44692379.6 | -142270953.9 | градиент (нТ/м): 196250843.4 | 18159605.2 | 95346313.1 | -73356961.4
```
