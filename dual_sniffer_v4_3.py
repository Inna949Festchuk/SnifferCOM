#!/usr/bin/env python3
"""
dual_sniffer_v4_3.py
Исправлен расчёт магнитного склонения через pygeomag (офлайн).
Добавлена поддержка WMM.COF для работы без интернета.
"""
...
try:
    ...
    # Библиотека сама ищет WMM.COF в текущей директории.
    GEO_MAG = GeoMag()
except ImportError:
    sys.exit("❌ Установите: pip install pygeomag")
except Exception as e:
    # Если файл не найден, ошибка возникнет здесь при первом вызове calculate,
    # но лучше проверить наличие файла заранее для понятного сообщения.
    import os
    if not os.path.exists("WMM.COF"):
        sys.exit("❌ Файл WMM.COF не найден в текущей папке!\n💡 Скачайте его и положите рядом со скриптом.")
    else:
        sys.exit(f"❌ Ошибка инициализации pygeomag: {e}")

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', datefmt='%H:%M:%S')
logger = logging.getLogger('dual_sniffer_v4_3')
...
def get_declination(lat: float, lon: float, year: int = 2026) -> float:
    ...
    try:
        # Пробуем вариант с именованными аргументами (новые версии)
        try:
            res = GEO_MAG.calculate(latitude=lat, longitude=lon, altitude=0.0, year=float(year))
            return res.d
        except TypeError:
            pass
        
        # Пробуем вариант с позиционными аргументами (старые версии)
        try:
        ...
        # Пробуем вариант с другими именами (альтернативные форки)
        try:
            res = GEO_MAG.calculate(lat=lat, lon=lon, alt=0.0, time=float(year))
            return res.d
        except TypeError:
            pass

        logger.warning("⚠️ Не удалось подобрать параметры для pygeomag. Проверьте документацию вашей версии.")
        return 0.0
...
def parse_args():
    p = argparse.ArgumentParser(description='Dual Sniffer v4.3 (pygeomag offline declination)')
...
def main():
    args = parse_args()
    logger.info(f"Dual Sniffer v4.3 | Sensors:{args.sensor_port}@{args.sensor_baud} | GPS:{args.gps_port}@{args.gps_baud}")
    logger.info(f"Geometry: forward={args.forward_offset}m, spacing={args.spacing}m")
    logger.info("⏱️ Синхронизация: спутниковое UTC (NMEA) | Магнитное склонение: pygeomag (WMM2020, офлайн)")
